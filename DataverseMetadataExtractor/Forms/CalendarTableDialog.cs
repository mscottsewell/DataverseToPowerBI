    ...
    public class CalendarTableDialog : Form
    {
        ...
        private void LoadAllDateTimeFields()
        {
            ...
            foreach (var tableName in _selectedTables.Keys.OrderBy(k => k))
            {
                ...
                {
                    // More robust DateTime field detection
        ...
        private void CboTable_SelectedIndexChanged(object? sender, EventArgs e)
        {
            ...
            if (cboTable.SelectedItem is ComboItem selected)
            {
                ...
                if (_attributeDisplayInfo.TryGetValue(tableName, out var attrs))
                {
                    ...
                    {
                        // Debug: Show all attribute types for this table
                        Services.DebugLogger.Log("  All attribute types for this table:");
                        foreach (var attr in attrs.Values.Take(10))
                        {
                            Services.DebugLogger.Log($"    - {attr.LogicalName}: '{attr.AttributeType}'");
                        }
                    }
                }
                else
